...
本文记录 v2.2 的数据口径、因子研究、信号合成、组合优化、执行假设和评价方法。公开结果来自指纹校验后的聚合制品；行情、信号、持仓和成交明细不随仓库分发。
...
发布方法采用的折内硬门槛如下；字段均来自 [年度研究配置](configs/studies/a2_factor_expansion_v1.yaml)，而不是报告生成时追加的过滤条件。

| 检查项 | 门槛 | 配置字段 |
|---|---:|---|
| 因子覆盖率 / 有效日期比例 | 均不低于 80% | `min_coverage` / `min_active_date_rate` |
| 有效截面股票数 / IC 日期数 | 均不少于 100 | `min_cross_section` / `min_ic_dates` |
| 平均定向 IC / 方向一致率 | 不低于 0 / 50% | `min_mean_directed_ic` / `min_direction_consistency` |
| Newey–West t 值 / BH q 值 | 不低于 0 / 不高于 0.20 | `min_newey_west_t` / `max_bh_q_value` |
| 费后 Q5−Q1 均值 / 正收益比例 | 不低于 0 / 50% | `min_mean_net_quintile_spread` / `min_net_spread_consistency` |
| 五分组单调性 | 不低于 0 | `min_quintile_monotonicity` |
| 四段入选频率 / IC 与价差联合通过频率 | 均不低于 50% | `min_segment_selection_frequency` / `min_joint_segment_frequency` |
| 相邻截面排名变化率 | 不高于 35% | `max_score_churn` |
| 绝对相关系数聚类阈值 | 0.75 | `max_abs_correlation` |
...
滚动岭回归先按决策日对合成得分做稳健标准化，再映射为未来 5 日预期主动收益。每个决策日在回归中具有相同总权重：

```math
\widehat b_t
=\arg\min_b\sum_{(i,u)\in\mathcal T_t}
\omega_{i,u}\left(y_{i,u}-b\widetilde s_{i,u}\right)^2+10b^2,
\qquad
\widehat\mu_{i,t}=\widehat b_t\widetilde s_{i,t}.
```

其中 $`\widetilde s`$ 为逐日中位数—中位数绝对偏差标准分，$`\omega`$ 使每个训练截面的权重和相同。岭惩罚系数 10 随发布配置固定，六个年度折不分别寻优；它作用于无截距单变量回归，以限制收益换算斜率。训练标签缩尾至 ±20%，得分缩尾至 ±3，回归斜率和单票预期收益分别限制在 ±5% 和 ±10%。每次拟合保存训练区间、最后成熟标签日、系数和样本数。
...
156 个训练截面、相关阈值 0.75、岭惩罚 10、风险厌恶系数 3、主动偏离惩罚 0.01 以及各项截断和组合约束，在生成六个年度折时由同一组配置固定，不按评价年度的结果重新估计。筛选与信号参数见 [方法配置](configs/studies/a2_factor_expansion_v1.yaml)，风险与组合参数见 [基础配置](configs/extended_validation_2023_2025.yaml)，发布门槛见 [年度配置](configs/annual/a2_factor_expansion_v1.yaml)。

这些参数是在 2020—2025 方法研究过程中形成的，因此“折内固定”不等于独立样本外预登记。项目没有对全部参数做高维网格寻优；现有敏感性证据来自风险模型、因子扩展和信号合成的有界对照实验。第 7.4 节保留了没有进入发布方案的结果。
...
少于 120 个有效观测的股票使用 80% 年化波动率回退，目标权重不得超过调仓前权重与基准权重的较大者。β 使用 60 日个股与中证 500 收益协方差估计；缺失时回退到 1。
...
式中的 $`\Sigma`$ 是日频协方差，乘以 5 是把风险项换算到与预期收益一致的 5 个交易日持有期；年化跟踪误差约束则另按 252 个交易日换算。基准参数为风险厌恶系数 3、主动偏离惩罚 0.01。约束如下：
...
引擎没有单独维护 A 股 T+1 可售库存。常规调仓间隔为 5 个开放交易日，每个执行日只在开盘撮合一次，并按先卖后买处理，因此当前路径不会卖出当日新买仓位；若扩展为日内或次日再次调仓，则必须补充显式的 T+1 库存约束。
...
| 滚动 β 偏离 95% 分位数 | 不高于 0.100 | 0.163 |
...
- 发布组合使用 Ledoit–Wolf 收缩协方差，不是结构化多因子风险模型；报告也没有给出完整的事后风格收益归因。
- 容量约束按 1 亿元组合规模和单次 5% ADV 参与率计算，没有给出多规模容量曲线，也没有建模策略拥挤。
- 连续份额记账尚未覆盖 100 股整数手、显式 T+1 可售库存和完整公司行动台账。
...
`book_to_price` 与 `book_to_market` 都描述账面价值相对价格，但数据口径不同：前者由日频 PB 指标计算，后者由当时可得财务权益和市值计算。保留两个名称是为了区分来源和可得时点；折内相关性聚类会阻止高度相关的重复暴露同时入选。
...
主动净值采用组合净值与基准净值之比；信息比率为主动日收益均值除以主动日收益标准差并按 252 年化。CAPM α 和 β 由组合日收益对基准日收益的含截距回归得到。

## 附录 C：术语

| 术语 | 本文口径 |
|---|---|
| Rank IC | 同一截面内预测排序与未来主动收益排序的 Spearman 相关系数 |
| IR | 年化主动收益均值除以年化跟踪误差，即信息比率 |
| ADV | 决策日可得的过去 20 个交易日平均成交额 |
| Q5−Q1 | 因子得分最高五分位组合减最低五分位组合的收益 |
| TTM | 截至可得报告期的连续四季流量合计 |
| Newey–West | 对时间序列自相关和异方差稳健的统计量修正 |
| 主动收益 | 组合收益相对中证 500 基准收益；累计口径采用两者净值之比 |
