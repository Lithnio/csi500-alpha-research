...
![最终留出期净值、相对净值差与回撤](docs/assets/final-holdout.png)
...
运行制品、研究输入和协议锁均通过哈希核验；关键指标由逐日净值与成交记录独立复算，最大差异为 0。公开聚合结果见 [final-holdout-summary.json](docs/assets/final-holdout-summary.json)，完整性检查见 [final-holdout-audit.json](docs/assets/final-holdout-audit.json)。
...
![IC 合成消融](docs/assets/innovation-ablation.png)
...
![扩展验证期净值与回撤](docs/assets/backtest-overview.png)
...
![成本与容量压力测试](docs/assets/stress-analysis.png)

扩展验证的聚合指标见 [public-summary.json](docs/assets/public-summary.json)，图表来源和输出哈希见 [report-manifest.json](docs/assets/report-manifest.json)。
